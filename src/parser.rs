...
fn simd_detect_rustdoc_format_version(path: &Path, file_data: &str) -> anyhow::Result<u32> {
    let version =
...
fn simd_parse_or_report_error<T>(
    path: &Path,
    file_data: &str,
    format_version: u32,
) -> anyhow::Result<T>
...
{
    simd_json::serde::from_slice(&mut file_data.as_bytes().to_owned()).with_context(|| {
        format!(
            "unexpected parse error for v{format_version} rustdoc for file {}",
            path.display()
        )
...
pub fn simd_load_rustdoc(path: &Path) -> anyhow::Result<VersionedCrate> {
    ...
    let format_version = simd_detect_rustdoc_format_version(path, &file_data)?;
